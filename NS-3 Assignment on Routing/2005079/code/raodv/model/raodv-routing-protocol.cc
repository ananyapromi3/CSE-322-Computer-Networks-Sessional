...
RoutingProtocol::Recvraodv(Ptr<Socket> socket)
{
    ...
    case raodvTYPE_REV: {
        RecvRevocation(packet, receiver, sender);
        break;
    }
    }
}
...
RoutingProtocol::RecvRequest(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    ...
    //     // **Initiate Reverse Broadcast**
    //     NS_LOG_DEBUG("Initiating reverse broadcast to origin: " << origin);
    //     Simulator::Schedule(Time(Seconds(0)),
    //                         &RoutingProtocol::RecvRevocation,
    //                         this,
    //                         p,
    //                         rreqHeader.GetDst(),
    //                         origin); // Destination becomes receiver, Origin becomes source
    //     return;
    // }
    ...
    if (IsMyOwnAddress(rreqHeader.GetDst()))
    {
        ...
        // return;
        rreqHeader.SetOrigin(rreqHeader.GetDst());
        rreqHeader.SetDst(origin);
        ...
        Simulator::Schedule(Time(Seconds(0)),
                            ...
                            p,
                            rreqHeader.GetDst(),
                            rreqHeader.GetOrigin()); // Destination becomes receiver, Origin becomes source
...
void
RoutingProtocol::RecvRevocation(Ptr<Packet> p, Ipv4Address receiver, Ipv4Address src)
{
    NS_LOG_FUNCTION(this);
    NS_LOG_DEBUG("Received reverse broadcast from " << src << " to " << receiver);

    RreqHeader revocationHeader;
    p->RemoveHeader(revocationHeader);

    uint32_t revocationId = revocationHeader.GetId();
    Ipv4Address origin = revocationHeader.GetOrigin();

    // Check if this node is the source (origin) of the reverse broadcast
    if (IsMyOwnAddress(origin))
    {
        NS_LOG_DEBUG("I am the origin of the reverse broadcast. Stopping propagation.");
        // Process the revocation here, e.g., update routing table or perform cleanup
        // HandleRevocation(
        //     revocationHeader); // Implement `HandleRevocation` for any necessary actions
        return;
    }

    // Check for duplicate revocation packets
    if (m_rreqIdCache.IsDuplicate(origin, revocationId))
    {
        NS_LOG_DEBUG("Ignoring duplicate reverse broadcast packet");
        return;
    }

    // Increment the hop count for the revocation message
    uint8_t hop = revocationHeader.GetHopCount() + 1;
    revocationHeader.SetHopCount(hop);

    // Add or update reverse route for the origin
    RoutingTableEntry toOrigin;
    if (!m_routingTable.LookupRoute(origin, toOrigin))
    {
        Ptr<NetDevice> dev = m_ipv4->GetNetDevice(m_ipv4->GetInterfaceForAddress(receiver));
        RoutingTableEntry newEntry(
            ...
            /*dst=*/origin,
            /*vSeqNo=*/true,
            /*seqNo=*/revocationHeader.GetOriginSeqno(),
            /*iface=*/m_ipv4->GetAddress(m_ipv4->GetInterfaceForAddress(receiver), 0),
            /*hops=*/hop,
            /*nextHop=*/src,
            /*lifetime=*/Time(2 * m_netTraversalTime - 2 * hop * m_nodeTraversalTime));
    ...
    else
    {
        if (toOrigin.GetValidSeqNo())
        {
            if (int32_t(revocationHeader.GetOriginSeqno()) - int32_t(toOrigin.GetSeqNo()) > 0)
            {
                toOrigin.SetSeqNo(revocationHeader.GetOriginSeqno());
            }
        }
        else
        {
    ...
    // Forward the revocation broadcast to neighbors
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); j++)
    {
        Ptr<Socket> socket = j->first;
        Ipv4InterfaceAddress iface = j->second;
        ...
        // ttl.SetTtl(tag.GetTtl() - 1);
        // packet->AddPacketTag(ttl);
        packet->AddHeader(revocationHeader);
        TypeHeader tHeader(raodvTYPE_REV); // Reverse broadcast
        packet->AddHeader(tHeader);
        // Send reverse broadcast
        Ipv4Address destination;
        if (iface.GetMask() == Ipv4Mask::GetOnes())
        {
            destination = Ipv4Address("255.255.255.255");
    ...
    NS_LOG_DEBUG("Reverse broadcast forwarded from " << src << " to " << receiver);
}
