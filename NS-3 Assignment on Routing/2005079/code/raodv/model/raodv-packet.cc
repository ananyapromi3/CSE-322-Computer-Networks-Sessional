...
// RREV
//-----------------------------------------------------------------------------
RrevHeader::RrevHeader(uint8_t flags,
                       uint8_t reserved,
                       uint8_t hopCount,
                       uint32_t requestID,
                       Ipv4Address dst,
...
NS_OBJECT_ENSURE_REGISTERED(RrevHeader);

TypeId
RrevHeader::GetTypeId()
{
    static TypeId tid = TypeId("ns3::raodv::RrevHeader")
                            .SetParent<Header>()
                            .SetGroupName("raodv")
                            .AddConstructor<RrevHeader>();
    return tid;
}

TypeId
RrevHeader::GetInstanceTypeId() const
{
    return GetTypeId();
}

uint32_t
RrevHeader::GetSerializedSize() const
{
    return 23;
}

void
RrevHeader::Serialize(Buffer::Iterator i) const
{
    i.WriteU8(m_flags);
    i.WriteU8(m_reserved);
    i.WriteU8(m_hopCount);
...
uint32_t
RrevHeader::Deserialize(Buffer::Iterator start)
{
    Buffer::Iterator i = start;
    m_flags = i.ReadU8();
    m_reserved = i.ReadU8();
...
void
RrevHeader::Print(std::ostream& os) const
{
    os << "RREQ ID " << m_requestID << " destination: ipv4 " << m_dst << " sequence number "
       << m_dstSeqNo << " source: ipv4 " << m_origin << " sequence number " << m_originSeqNo
       << " flags:"
...
std::ostream&
operator<<(std::ostream& os, const RrevHeader& h)
{
    h.Print(os);
    return os;
}
...
void
RrevHeader::SetGratuitousRrep(bool f)
{
    if (f)
    {
        m_flags |= (1 << 5);
...
bool
RrevHeader::GetGratuitousRrep() const
{
    return (m_flags & (1 << 5));
}

void
RrevHeader::SetDestinationOnly(bool f)
{
    if (f)
    {
        m_flags |= (1 << 4);
...
bool
RrevHeader::GetDestinationOnly() const
{
    return (m_flags & (1 << 4));
}

void
RrevHeader::SetUnknownSeqno(bool f)
{
    if (f)
    {
        m_flags |= (1 << 3);
...
bool
RrevHeader::GetUnknownSeqno() const
{
    return (m_flags & (1 << 3));
}

bool
RrevHeader::operator==(const RrevHeader& o) const
{
    return (m_flags == o.m_flags && m_reserved == o.m_reserved && m_hopCount == o.m_hopCount &&
            m_requestID == o.m_requestID && m_dst == o.m_dst && m_dstSeqNo == o.m_dstSeqNo &&
            m_origin == o.m_origin && m_originSeqNo == o.m_originSeqNo);
