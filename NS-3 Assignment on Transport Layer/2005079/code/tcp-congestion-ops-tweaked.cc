...
NS_OBJECT_ENSURE_REGISTERED(TcpNewRenoTweaked);

TypeId
TcpNewRenoTweaked::GetTypeId()
{
    static TypeId tid = TypeId("ns3::TcpNewRenoTweaked")
                            .SetParent<TcpCongestionOps>()
                            .SetGroupName("Internet")
                            .AddConstructor<TcpNewRenoTweaked>();
    return tid;
}

TcpNewRenoTweaked::TcpNewRenoTweaked()
    : TcpCongestionOps()
{
    // NS_LOG_FUNCTION(this);
}

TcpNewRenoTweaked::TcpNewRenoTweaked(const TcpNewRenoTweaked& sock)
    : TcpCongestionOps(sock)
{
    NS_LOG_FUNCTION(this);
}
 ...
 */
uint32_t
TcpNewRenoTweaked::SlowStart(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
    NS_LOG_FUNCTION(this << tcb << segmentsAcked);
    ...
    // tweak
    while (segmentsAcked > 0 && tcb->m_cWnd < tcb->m_ssThresh)
    {
        tcb->m_cWnd += tcb->m_segmentSize;
        --segmentsAcked;

        // Ensure we don't overshoot excessively beyond the ssthresh
        if (tcb->m_cWnd > 0.75 * tcb->m_ssThresh)
        {
            NS_LOG_INFO("Approaching ssthresh; gradually slowing down slow start.");
            break;
        }
    }
 ...
 */
void
TcpNewRenoTweaked::CongestionAvoidance(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
    NS_LOG_FUNCTION(this << tcb << segmentsAcked);
    ...
    if (segmentsAcked > 0)
    {
        // Modify increment factor to adapt based on cWnd size
        double adder =
            static_cast<double>(tcb->m_segmentSize * tcb->m_segmentSize) / tcb->m_cWnd.Get();
        adder = std::max(1.0, adder * 1.25); // Increase more aggressively
 ...
 */
void
TcpNewRenoTweaked::IncreaseWindow(Ptr<TcpSocketState> tcb, uint32_t segmentsAcked)
{
    NS_LOG_FUNCTION(this << tcb << segmentsAcked);
...
std::string
TcpNewRenoTweaked::GetName() const
{
    return "TcpNewRenoTweaked";
}

uint32_t
TcpNewRenoTweaked::GetSsThresh(Ptr<const TcpSocketState> state, uint32_t bytesInFlight)
{
    NS_LOG_FUNCTION(this << state << bytesInFlight);
...
Ptr<TcpCongestionOps>
TcpNewRenoTweaked::Fork()
{
    return CopyObject<TcpNewRenoTweaked>(this);
}
